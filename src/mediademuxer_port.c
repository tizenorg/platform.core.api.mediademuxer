...
mediademuxer_src_type __md_util_media_type(char **uri)
{
	...
	if ((path = strstr((*uri), "file://"))) {
		...
		if (strlen(path)) {
			if (g_str_has_suffix
			    (g_ascii_strdown(*uri, strlen(*uri)),
			     ".ism/manifest")
			    ||
			    g_str_has_suffix(g_ascii_strdown
			                     (*uri, strlen(*uri)),
			                     ".isml/manifest")) {
		...
		if (file_stat == MD_ERROR_NONE) {
			...
			old_uristr =
			    (char *)g_malloc(sizeof(char) * (len_uri + 1));
...
int _md_util_parse(MMHandleType demuxer, const char *type)
{
	...
	media_type_string =
	    (char *)g_malloc(sizeof(char) * (lenght_string + 1));
	...
	new_demuxer->uri_src_media_type =
	    __md_util_media_type(&media_type_string);
