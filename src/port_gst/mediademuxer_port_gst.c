...
static int gst_demuxer_init(MMHandleType *pHandle)
{
	...
	*pHandle = (MMHandleType) new_mediademuxer;
	...
	new_mediademuxer->media_format_info = NULL;
	new_mediademuxer->media_format_info_count = 0;
	MEDIADEMUXER_FLEAVE();
	return ret;
ERROR:
	MEDIADEMUXER_FLEAVE();
                                      ...
                                      media_format_h *format, int index)
{
	...
	MEDIADEMUXER_CHECK_NULL(pHandle);
	mdgst_handle_t *new_mediademuxer = (mdgst_handle_t *) pHandle;
	media_format_h *media_format_info = NULL;;
	...
	if (index >= loop && index < 0)
		goto ERROR;
	if(new_mediademuxer->media_format_info == NULL){
           new_mediademuxer->media_format_info = (media_format_h *) g_malloc(sizeof(media_format_h) * loop);
	   new_mediademuxer->media_format_info_count = loop;
	}
	media_format_info = new_mediademuxer->media_format_info;
	if(media_format_info != NULL){
		ret = media_format_create(&media_format_info[index]);
	...
	MD_I("format[%p]\n", media_format_info[index]);
	if (temp->name[0] == 'a') {
		MD_I("Setting for Audio \n");
		_set_mime_audio(media_format_info[index], temp);
	} else if (temp->name[0] == 'v') {
		MD_I("Setting for Video \n");
		_set_mime_video(media_format_info[index], temp);
	} else {
		MD_I("Not supported so far\n");
	}
	*format = media_format_info[index];
	MEDIADEMUXER_FLEAVE();
	return ret;
ERROR:
	MEDIADEMUXER_FLEAVE();
