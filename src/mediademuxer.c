...
/**
* @file Media demuxer API file
* @brief This file contains all the APIs of media demuxer
*/
...
int mediademuxer_set_data_source(mediademuxer_h demuxer, const char *path)
{
	...
	if (handle && path && handle->demux_state == MEDIADEMUXER_IDLE) {
		...
		if (!path) {
			MD_E("[CoreAPI][%s] DEMUXER_ERROR_INVALID_OPERATION(0x%08x)", __FUNCTION__, MEDIADEMUXER_ERROR_INVALID_URI);
			return MEDIADEMUXER_ERROR_INVALID_URI;
		} else {
			if(handle->demux_state != MEDIADEMUXER_IDLE)
				return MEDIADEMUXER_ERROR_INVALID_STATE;
			MD_E("[CoreAPI][%s] DEMUXER_ERROR_INVALID_OPERATION(0x%08x)", __FUNCTION__, MEDIADEMUXER_ERROR_INVALID_OPERATION);
			return MEDIADEMUXER_ERROR_INVALID_OPERATION;
		}
	}
	return ret;
