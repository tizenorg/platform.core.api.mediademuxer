 ...
 * @brief general port based functions. sets sepcifif function pointers
 */
 ...
 * @pre It will be invoked when the error has occured if you register this callback using mediademuxer_set_error_cb().
...
typedef struct _media_port_demuxer_ops {
	...
	int (*get_track_info)(MMHandleType pHandle,
	...
	int (*read_sample)(MMHandleType pHandle,
	...
	int (*set_error_cb)(MMHandleType demuxer,
			md_error_cb callback, void* user_data);
 ...
 * @param   format             [out]   Handle of media_format
 *
 * @return  This function returns zero on success, or negative value with error code.
 *
 * @par Example
